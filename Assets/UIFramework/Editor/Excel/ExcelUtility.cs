...
{
    public List<Dictionary<string, object>> ConfigInfo = new List<Dictionary<string, object>>();
...
public class ExcelUtility 
{
    ...
    public void ConvertToJson(string JsonPath, Encoding encoding)
    {
        ...
        for (int i = 1; i < rowCount; i++)
        {
            ...
            //添加到表数据中
            jsonData.ConfigInfo.Add(row);
        }
        JsonSerializerSettings jsetting = new JsonSerializerSettings();
        jsetting.DefaultValueHandling = DefaultValueHandling.Ignore;

        //生成Json字符串
        string json = JsonConvert.SerializeObject(jsonData, Newtonsoft.Json.Formatting.Indented, jsetting);
        //写入文件
